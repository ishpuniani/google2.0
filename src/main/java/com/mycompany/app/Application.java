...
import com.mycompany.app.DocumentFiles.QueryFiles.QueryObject;
...
import static com.mycompany.app.ApplicationLibrary.*;
import static com.mycompany.app.DocumentFiles.FBIS.FBISProcessor.loadFBISDocs;
import static com.mycompany.app.DocumentFiles.FederalRegister.FedRegister.loadFedRegisterDocs;
import static com.mycompany.app.DocumentFiles.FinTimes.FinTimesLib.loadFinTimesDocs;
import static com.mycompany.app.DocumentFiles.LaTimes.LATimesParser.loadLaTimesDocs;
import static com.mycompany.app.DocumentFiles.QueryFiles.QueryLib.loadQueriesFromFile;
import static com.mycompany.app.RankAndAnalyzerFiles.RankAndAnalyzers.*;
			...
			for (QueryObject queryData : loadedQueries) {
				...
				if (queryData.getTitle().length() > 0) {
					...
					for (int hitIndex = 0; hitIndex < hits.length; hitIndex++) {
						...
						writer.println(queryData.getQueryNum() + ITER_NUM + indexSearcher.doc(hit.doc).get("docno") + " " + hitIndex
								+ " " + hit.score + ITER_NUM);
					}
				}
			}
	...
	private static List<String> splitNarrIntoRelNotRel(String narrative) {
		...
		while (bi.next() != BreakIterator.DONE) {
			...
			if (!sentence.contains("not relevant") && !sentence.contains("irrelevant")) {
				...
				irrelevantNarr.append(
						sentence.replaceAll("are also not relevant|are not relevant|are irrelevant|is not relevant|not|NOT", ""));
			}
			index = bi.current();
		}
