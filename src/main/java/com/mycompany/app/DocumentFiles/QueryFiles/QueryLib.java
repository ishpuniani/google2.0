package com.mycompany.app.DocumentFiles.QueryFiles;
...
public class QueryLib {
    ...
    public static List<QueryObject> loadQueriesFromFile() {
        ...
        try {
            ...
            while ((queryLine = bf.readLine()) != null) {
                ...
                if (queryLineTag != null && queryLineTag.equals(topTag)) { // if docLineTag isnt null and if it hasnt
                                                                           // changed
                    counter++;
    ...
    private static void populateQueryFields(String queryLineTag, String queryLine, QueryObject queryObject,
            int counter) {
        if (queryLineTag.equals(QueryTags.QUERY_NUMBER.getTag())) {
            ...
            queryObject
                    .setTitle(queryObject.getTitle() + " " + queryLine.replaceAll(QueryTags.QUERY_TITLE.getTag(), ""));
        } else if (queryLineTag.equals(QueryTags.QUERY_DESCRIPTION.getTag())) {
            queryObject.setDescription(queryObject.getDescription() + " "
                    + queryLine.replaceAll(QueryTags.QUERY_DESCRIPTION.getTag(), ""));
        } else if (queryLineTag.equals(QueryTags.QUERY_NARRATIVE.getTag())) {
            queryObject.setNarrative(
                    queryObject.getNarrative() + " " + queryLine.replaceAll(QueryTags.QUERY_NARRATIVE.getTag(), ""));
        } else {
