package com.mycompany.app.Parsers;
...
public class LaTimesParser {
	public static void main(String[] args) throws IOException {
		...
		File baseDir = new File(Constants.PARSED_DOCS_FILE_PATH + "latimes/");
		baseDir.mkdirs();

		List<String> results = new ArrayList<String>();
		// System.out.println(Constants.DATASET_FILE_PATH + "latimes/");
		File[] files = new File(Constants.DATASET_FILE_PATH + "latimes/").listFiles();
		for (File file : files) {
			try {
				if (file.isFile()) {
					...
					for (Element e : docs) {
						...
						File result = new File(Constants.PARSED_DOCS_FILE_PATH + "latimes/" + DocNo);
